...
有一个max最大值，然后用max开一个fd_set用来保存标记。通过select方法讲fd_set由用户态拷贝到内核态。阻塞。一旦由数据到了，select方法返回，并且会在fd_set中标记有数据的文件描述符。然后开始遍历fd_set，遇到了标记就处理。
...
poll阻塞，然后有数据来了，就返回，然后就是遍历pollfds数组，检查revents，如果有就读，并且把它重置为0。那么每次的pollfds都是一样的，就可以重复利用了

改进：
- 用pollfds数组，大小远远不止1024，所以就等于没有max限制了
- 实现了数组的重用
